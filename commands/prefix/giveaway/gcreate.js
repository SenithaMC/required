  ...
  async execute(message, args) {
    if (!message.member.permissions.has(PermissionsBitField.Flags.ManageMessages)) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> You need the `Manage Messages` permission to create giveaways.')
        ]
      });
    }
    ...
    if (args.length < 2) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Invalid syntax. Usage: `gcreate [winners] <prize> <time> [@role]`')
            .addFields(
              { name: 'Examples', value: 
      ...
      if (!role) {
        return message.channel.send({
          embeds: [
            new EmbedBuilder()
              .setColor(0xFF0000)
              .setDescription('<:mc_white_cross:1411727598840451174> Invalid role mentioned.')
          ]
        });
      }
    ...
    if (!prize) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Prize is required.')
        ]
      });
    }
    ...
    if (!time) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Time is required. Format: 1s, 5m, 3h, 2d')
        ]
      });
    }
    ...
    if (!duration || duration < 1000) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Invalid time format. Use: s (seconds), m (minutes), h (hours), d (days)')
        ]
      });
    }
    ...
    try {
      ...
      await db.pool.execute(
        `INSERT INTO giveaways (messageId, channelId, guildId, prize, winners, endTime, role, participants, hostId) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [giveawayMessage.id, message.channel.id, message.guild.id, prize, winners, new Date(endTime), role.id, JSON.stringify([]), message.author.id]
      );
      ...
      collector.on('collect', async i => {
        try {
          ...
          await db.pool.execute(
            'UPDATE giveaways SET participants = ? WHERE messageId = ?',
            [JSON.stringify(participants), i.message.id]
          );
      ...
      await message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Failed to create giveaway. Please try again.')
        ]
      });
    }
  },
