    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.ManageChannels)) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:error:1416752161638973490> You do not have permission to manage channels.')
        ]
      });
    }
    ...
    try {
      ...
      await message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:error:1416752161638973490> Failed to lock the channel. Do I have permission?')
        ]
      });
    }
  }
