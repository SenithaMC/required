    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.BanMembers)) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> You need the `Ban Members` permission to use this command.')
        ]
      });
    }
    ...
    if (!message.mentions.roles.size) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Please mention a role to mass ban.')
    ...
    if (targetRole.id === message.guild.id) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> You cannot mass ban the @everyone role.')
        ]
      });
    }
  ...
  handleComponent: async (interaction) => {
    ...
    if (interaction.customId === 'massban_cancel') {
      ...
      await interaction.update({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Mass ban cancelled.')
        ],
        components: []
      });
      return true;
    ...
    if (interaction.customId.startsWith('massban_confirm_')) {
      ...
      const resultEmbed = new EmbedBuilder()
        .setTitle('<:mc_white_check_mark:1410629081845989386> Mass Ban Complete')
