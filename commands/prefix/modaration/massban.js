    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.BanMembers)) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:error:1416752161638973490> You need the `Ban Members` permission to use this command.')
        ]
      });
    }
    ...
    if (!message.mentions.roles.size) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:error:1416752161638973490> Please mention a role to mass ban.')
    ...
    if (targetRole.id === message.guild.id) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:error:1416752161638973490> You cannot mass ban the @everyone role.')
        ]
      });
    }
    ...
    if (interaction.customId === 'massban_cancel') {
      ...
      await interaction.update({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:error:1416752161638973490> Mass ban cancelled.')
        ],
        components: []
      });
      return true;
