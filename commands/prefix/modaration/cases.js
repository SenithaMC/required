    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.ViewAuditLog)) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> You do not have permission to view cases of members.')
        ]
      });
    }
    ...
    try {
      ...
      await message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> There was an error fetching cases.')
        ]
      });
    }
  },
...
async function sendCasesPage(messageOrInteraction, targetUserOrId, page = 1, isInteraction = false) {
  ...
  if (!targetUser) {
    const embed = new EmbedBuilder()
      .setColor(0xFF0000)
      .setDescription('<:mc_white_cross:1411727598840451174> User not found.');
  ...
  try {
    ...
    let msg;
    if (isInteraction) {
      msg = await messageOrInteraction.editReply({ 
        embeds: [embed], 
        components: [buttons] 
      });
    } else {
