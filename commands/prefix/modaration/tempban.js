...
module.exports = {
  ...
  execute: async (message, args) => {
    if (!message.member.permissions.has(PermissionsBitField.Flags.BanMembers)) {
      return message.channel.send('<:mc_white_cross:1411727598840451174> You do not have permission to ban members.');
    }

    const user = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    if (!user) {
      return message.channel.send('<:mc_white_cross:1411727598840451174> Please specify a valid user to ban.');
    }

    const duration = args[1];
    if (!duration) return message.channel.send('<:mc_white_cross:1411727598840451174> Please provide a duration (e.g., 1d, 2h, 30m).');
    ...
    if (!match) {
      return message.channel.send('<:mc_white_cross:1411727598840451174> Invalid duration format. Use format like: 1d (day), 2h (hour), 30m (minute)');
    }
    ...
    switch(unit) {
      ...
      default: return message.channel.send('<:mc_white_cross:1411727598840451174> Invalid time unit. Use d, h, or m.');
    }
    ...
    try {
      ...
      console.error(err);
      message.channel.send('<:mc_white_cross:1411727598840451174> Failed to ban user. Do I have permission?');
    }
  }
};
