  ...
  async execute(message, args) {
    if (!message.member.permissions.has(PermissionsBitField.Flags.KickMembers)) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> You need the `Moderate Members` permission to warn users.')
        ]
      });
    }
    ...
    if (!message.mentions.users.size) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Please mention a user to warn.')
    ...
    if (!reason) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Please provide a reason for the warning.')
            .addFields({ name: 'Usage', value: this.usage })
        ]
      });
    }
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setTitle('<:mc_white_check_mark:1410629081845989386> User Warned')
      ...
      await message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> There was an error warning the user.')
        ]
      });
    }
  },
