    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.KickMembers)) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> You need the `Kick Members` permission to use this command.')
        ]
      });
    }
    ...
    if (!message.mentions.roles.size) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Please mention a role to mass kick.')
    ...
    if (targetRole.id === message.guild.id) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> You cannot mass kick the @everyone role.')
        ]
      });
    }
  ...
  handleComponent: async (interaction) => {
    ...
    if (interaction.customId === 'masskick_cancel') {
      ...
      await interaction.update({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Mass kick cancelled.')
        ],
        components: []
      });
      return true;
    ...
    if (interaction.customId.startsWith('masskick_confirm_')) {
      ...
      if (!targetRole) {
        ...
        await interaction.editchannel.send({
          embeds: [
            new EmbedBuilder()
              .setColor(0xFF0000)
              .setDescription('<:mc_white_cross:1411727598840451174> The role for this mass action no longer exists.')
          ],
          components: []
        });
        return true;
      ...
      const resultEmbed = new EmbedBuilder()
        .setTitle('<:mc_white_check_mark:1410629081845989386> Mass Kick Complete')
