    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.ManageMessages)) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> You need the `Manage Messages` permission to reroll giveaways.')
        ]
      });
    }
    ...
    if (!args[0]) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Please provide a message ID of the giveaway.')
            .addFields(
    ...
    try {
      ...
      if (giveawayRows.length === 0) {
        return message.channel.send({
          embeds: [
            new EmbedBuilder()
              .setColor(0xFF0000)
              .setDescription('<:mc_white_cross:1411727598840451174> Could not find a giveaway with that message ID.')
          ]
        });
      }
      ...
      if (!giveaway.ended) {
        return message.channel.send({
          embeds: [
            new EmbedBuilder()
              .setColor(0xFF0000)
              .setDescription('<:mc_white_cross:1411727598840451174> This giveaway is still running. Use `gend` to end it first.')
          ]
        });
      }
      ...
      if (participants.length === 0) {
        return message.channel.send({
          embeds: [
            new EmbedBuilder()
              .setColor(0xFF0000)
              .setDescription('<:mc_white_cross:1411727598840451174> No participants in this giveaway to reroll.')
          ]
        });
      }
      ...
      await message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:mc_white_cross:1411727598840451174> Failed to reroll giveaway. Please try again.')
        ]
      });
    }
  },
