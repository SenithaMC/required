    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.KickMembers)) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:error:1416752161638973490> You need the `Kick Members` permission to use this command.')
        ]
      });
    }
    ...
    if (!message.mentions.roles.size) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:error:1416752161638973490> Please mention a role to mass kick.')
    ...
    if (targetRole.id === message.guild.id) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:error:1416752161638973490> You cannot mass kick the @everyone role.')
        ]
      });
    }
    ...
    if (interaction.customId === 'masskick_cancel') {
      ...
      await interaction.update({
        embeds: [
          new EmbedBuilder()
            .setColor(0xFF0000)
            .setDescription('<:error:1416752161638973490> Mass kick cancelled.')
        ],
        components: []
      });
      return true;
      ...
      if (!targetRole) {
        ...
        await interaction.editchannel.send({
          embeds: [
            new EmbedBuilder()
              .setColor(0xFF0000)
              .setDescription('<:error:1416752161638973490> The role for this mass action no longer exists.')
          ],
          components: []
        });
        return true;
